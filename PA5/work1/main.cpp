...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
    ...
    for (auto &kp : keypoints) {
        ...
        m_01 = 0;   m_10 = 0;   flag = false;
        cv::Rect rect_image(0, 0, image.cols, image.rows);
        for(int range_row = -half_patch_size; range_row < half_patch_size; range_row++){
            for(int range_col = -half_patch_size; range_col < half_patch_size; range_col++){
                cv::Point point(cvRound(kp.pt.x + range_col), cvRound(kp.pt.y + range_row));
                if(rect_image.contains(point)){
                //    m_01 += point.y * (int)image.at<uchar >(cvRound(point.y), cvRound(point.x));
                //    m_10 += point.x * (int)image.at<uchar >(cvRound(point.y), cvRound(point.x));
                    m_01 += range_row * (int)image.at<uchar >(cvRound(point.y), cvRound(point.x));
                    m_10 += range_col * (int)image.at<uchar >(cvRound(point.y), cvRound(point.x));
                }else{
                    flag = true;
                    break;
                }
        ...
        if(flag){
            ...
            kp.angle = (float)std::atan2(m_01, m_10) * 180.0 / pi;
        }
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    //cout << "image.cols" << image.cols << " " << "image.row" << image.rows << endl;
    for (auto &kp: keypoints) {
        static int count = 0;
        DescType d(256, false);
        cv::Rect rect = cv::Rect(0, 0, image.cols, image.rows);
        double cos_theta = std::cos(kp.angle * pi / 180);
        double sin_theta = std::sin(kp.angle * pi / 180);
        for (int i = 0; i < 256; i++) {
            // START YOUR CODE HERE (~7 lines)
            d[i] = false;  // if kp goes outside, set d.clear()
            cv::Point p, q;
            p.x = cvRound(cos_theta * ORB_pattern[i*4 + 0] - sin_theta * ORB_pattern[i*4 + 1]) + cvRound(kp.pt.x);
            p.y = cvRound(sin_theta * ORB_pattern[i*4 + 0] + cos_theta * ORB_pattern[i*4 + 1]) + cvRound(kp.pt.y);
            q.x = cvRound(cos_theta * ORB_pattern[i*4 + 2] - sin_theta * ORB_pattern[i*4 + 3]) + cvRound(kp.pt.x);
            q.y = cvRound(sin_theta * ORB_pattern[i*4 + 2] + cos_theta * ORB_pattern[i*4 + 3]) + cvRound(kp.pt.y);
            if(rect.contains(p) && rect.contains(q)){
                if((int)image.at<uchar >(p) > (int)image.at<uchar >(q)){
                    d[i] = false;
                }else{
                    d[i] = true;
                }
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    for(int i = 0; i < desc1.size(); i++){
        ...
        for(int j = 0; j < desc2.size(); j++){
            ...
            distance = 0;
            for(int n = 0; n < 256; n++){
                if(desc1[i][n] != desc2[j][n])  distance++;
            }
            if(distance < min_dis){
                min_dis = distance;
                mark_j = j;
        ...
        if(min_dis <= d_max){
            cv::DMatch match(i, mark_j, min_dis);
